...
Des applications Shiny sont développées pour faciliter l'accès aux données intégrer dans [https://data-snot.cnrs.fr/](https://data-snot.cnrs.fr/). Les applications Shiny sont diffusées grâce à shiny-server installé sur la même VM que le serveur tomcat de l'application data-snot.cnrs.fr. Son IP est `163.9.41.45` et il est accessible avec l'utilisateur tomcat en ssh avec cette commande :

```
ssh tomcat@163.9.41.45
```
...
- Applications sur le port **3939** : ensemble des applications internes. Elles sont lancées par l'utilisateur *tomcat* dans le répertoire `~/ShinyApps/nom_repertoire` et auront comme url cette logique : `http://data-snot.cnrs.fr:3838/tomcat/nom_repertoire/`.
- Applications sur le port **3838** : Applications dédiées au grand public. Pour le moment, il s'agit de l'application `data-access` et de sa version de développement `data-access-snapshot` (pour visualiser et extraire les données du SNOT). Les applications sont stockées dans `/srv/shiny-server/nom_repertoire` et auront ce type d'url `http://data-snot.cnrs.fr:3838/nom_repertoire/`. 
