...
Le module est compilé dans `~/PivotMetadata_snot/` et le programme s'appelle : `jax-d-swarm.jar`. Le module est relancé automatiquement à chaque redémarrage du serveur 163.9.41.45 grâce à la configuration du /etc/crontab :

```{r, engine = 'bash',class.source='klippy',eval=FALSE}
@reboot         tomcat    /opt/default-jdk/bin/java -jar ~/PivotMetadata_snot/jax-d-swarm.jar
``` 
...
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
nohup java -jar ~/PivotMetadata_snot/jax-d-swarm.jar &
```
