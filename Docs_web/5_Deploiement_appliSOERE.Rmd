...
git clone https://forge-osuc.cnrs-orleans.fr/git/sie-sno-tourbiere prod
```
...
L'ensemble des fichiers de configuration sont stockés dans un répertoire `conf` non versionné à la racine du repertoire `devel` : `~/devel/conf`
Il contient : 
...
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cp ~/devel/conf/pom_prod.xml ~/devel/prod/Developpement/SI/app_snot/pom.xml
```

On compile ensuite l'application
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cd ~/devel/prod/Developpement/SI/app_snot/
...
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cp ~/devel/prod/Developpement/SI/app_snot/snot-web/target/snot-web-1902.01-SNAPSHOT.war /opt/tomcat8/webapps/snot.war
```
...
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cp ~/devel/conf/pom_dev.xml ~/devel/dev/Developpement/SI/app_snot/pom.xml
```

On compile ensuite l'application

```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cd ~/devel/dev/Developpement/SI/app_snot/
mvn clean install -D maven.test.skip.exec=true
```
...
```{r, engine = 'bash',class.source='klippy',eval=FALSE}
cp ~/devel/dev/Developpement/SI/app_snot/snot-web/target/snot-web-1902.01-SNAPSHOT.war /opt/tomcat8/otherinstances/snapshot/webapps/snot-web-1902.01-SNAPSHOT.war
```
