...
Voici un exemple d'arborescence du stockage dans le serveur FTP `SRV-SO` du type de données `doc` et des types de données complémentaires `fluo` et `debit` pour la station multiparamètres `sm_exutoire` de la tourbière de la Guette.
...
- 1_BRUTE_doc (*recommandé*) : Fichiers bruts regroupant toutes les fichiers en provenance de la station sm_exutoire. Ce répertoire contient les différents types de données utilisés pour construire les variables cible
